...
func TestNewApiRequest(t *testing.T) {
	...
	for _, it := range testCases {
		...
		if nil != err {
			t.Error(fmt.Sprintf("failed url construction for %v, got: %v", it[0], err))
			return
		}
		req, err := NewApiRequest(testUrl, method, testUser)
		if nil != err {
			t.Error(fmt.Sprintf("unexpected path %v failed validation, got: %v", testUrl.Path, err))
			return
		}
		if it[1] != req.Verb {
			t.Error(fmt.Sprintf("unexpected path %v verb, got: %v", it[0], req.Verb))
			return
		}
		if "@user" != req.Workspace {
			t.Error(fmt.Sprintf("unexpected path %v workspace, got: %v", it[0], req.Workspace))
			return
		}
		if it[2] != req.Key {
			t.Error(fmt.Sprintf("unexpected path %v key, got: %v", it[0], req.Key))
			return
		}
		if testUser != req.Cx.User {
			t.Error(fmt.Sprintf("unexpected path %v user, got: %v", it[0], req.Cx.User))
			return
		}
	}
	...
	for _, it := range invalidTests {
		...
		if nil == err {
			t.Error(fmt.Sprintf("path should have failed validation: %v", it))
			return
		}
	}
}
...
func doApiRequest(t *testing.T, verb string) (*ApiResult, error) {
	...
	if nil != err {
		t.Error(fmt.Sprintf("failed url construction for upload/@user, got: %v", err))
		return nil, err
	}
	...
	if "ok" != result.Result {
		t.Error(fmt.Sprintf("unexpected path %v failed handling, got: %v", testUrl.Path, err))
		return nil, err
	}
	json, err := json.MarshalIndent(result, "", "  ")
	if nil != err {
		t.Error(fmt.Sprintf("failed to marshall json for %v, got: %v", result, err))
		return nil, err
	}
...
func doUploadApiRequest(t *testing.T) (bool) {
	...
	if nil != err {
		t.Error(fmt.Sprintf("failed to setup upload request, got: %v", err))
		return false
	}
	resp, err := httpClient.Do(req)
	if nil != err {
		t.Error(fmt.Sprintf("failed to upload test content, got: %v", err))
		return false
	}
	defer resp.Body.Close()
	if resp.StatusCode != 200 {
		t.Error(fmt.Sprintf("non-200 upload response, got: %v", resp))
		return false
	}
	return true
}
...
func doDownloadApiRequest(t *testing.T) (bool) {
	...
	if nil != err {
		t.Error(fmt.Sprintf("failed to download test content, got: %v", err))
		return false
	}
	defer resp.Body.Close()
	if resp.StatusCode != 200 {
		t.Error(fmt.Sprintf("non-200 download response, got: %v", resp))
		return false
	}
	...
	if body != testMessage {
		t.Error(fmt.Sprintf("download does not match upload: %v ?= %v", body, testMessage))
		return false
	}
	return true
}
...
func doListApiRequest(t *testing.T) (bool) {
	...
	if len(listData.Objects) != 1 || listData.Objects[0].WorkspaceKey != key {
		t.Error(fmt.Sprintf("failed to list object, got: %v", listData.Objects))
		return false
	}
