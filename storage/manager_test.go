...
func TestMgrMakePath(t *testing.T) {
	...
	for _, it := range testCases {
		...
		if nil != err {
			t.Error(fmt.Sprintf("unexpected path failed validation, got: %v", err))
			return
		}
		expected := fmt.Sprintf("%v/%v/%v", testPrefix, testUser, it[1])
		if path != expected {
			t.Error(fmt.Sprintf("unexpected result: %v != %v", path, expected))
			return
		}
	}
	invalidTests := []string { 
...
func TestMgrList(t *testing.T) {
	...
	for _, it := range info.Objects {
		if (strings.Contains(it.WorkspaceKey, testUser)) {
			t.Error(fmt.Sprintf("workspace key should be below user level: %v", it.WorkspaceKey))
			return
		}
	}
	for _, it := range info.Prefixes {
		if (strings.Contains(it, testUser)) {
			t.Error(fmt.Sprintf("workspace prefix should be below user level: %v", it))
			return
		}
	}
}
...
func TestMgrUpDown(t *testing.T) {
	...
	for i := 0; i < 2; i += 1 {
		...
		if nil != err {
			t.Error(fmt.Sprintf("%v failed to setup upload request, got: %v", i, err))
			return
		}
		resp, err := httpClient.Do(req)
		if nil != err {
			t.Error(fmt.Sprintf("%v failed to upload test content, got: %v", i, err))
			return
		}
		defer resp.Body.Close()
		if resp.StatusCode != 200 {
			t.Error(fmt.Sprintf("%v non-200 upload response, got: %v", i, resp))
			return
		}
	}
	...
	if len(info.Objects) != 1 || info.Objects[0].WorkspaceKey != key {
		t.Error(fmt.Sprintf("failed to list object, got: %v", info.Objects))
		return
	}
	...
	if len(info.Objects) != 0 {
		t.Error(fmt.Sprintf("list still has deleted object, got: %v", info.Objects))
		return
	}
}
